...
firebase.initializeApp({
  ...
  authDomain: "habitdost-6ac79.firebaseapp.com",
  projectId: "habitdost-6ac79",
  storageBucket: "habitdost-6ac79.firebasestorage.app",
...
messaging.onBackgroundMessage((payload) => {
  ...
  self.registration.showNotification(title || 'HabitDost Reminder', {
    body: body || "Don't forget your habit today!",
    icon: icon || '/favicon.ico',
    badge: '/favicon.ico',
    tag: 'habitdost-reminder',
    ...
    event.waitUntil(
      clients.matchAll({ type: 'window', includeUncontrolled: true }).then(clientList => {
        for (const client of clientList) {
          if (client.url.includes('habitdost-dashboard') && 'focus' in client) return client.focus();
        }
        if (clients.openWindow) return clients.openWindow('./habitdost-dashboard.html');
      })
    );
  }
});
  ...
  reminderInterval = setInterval(() => {
    ...
    reminders.forEach(r => {
      if (r.time === currentTime) {
        self.registration.showNotification(`⏰ HabitDost Reminder`, {
